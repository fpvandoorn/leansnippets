...
open quotient eq circle sum sigma equiv function relation e_closure sigma.ops is_trunc
...
-- definition spokes_equiv {A : Type} {a : A} (p : a = a)
--   : (Σ(b : A), Π(z : S¹), circle.elim a p z = b) ≃ p = idp :=
-- calc
--   (Σ(b : A), Π(z : S¹), circle.elim a p z = b)
--         ≃ Σ(b : A) (q : a = b), pathover (λz, circle.elim a p z = b) q loop q
--   : sigma_equiv_sigma_right (λb, circle_pi_equiv _)
--     ... ≃ pathover (λz, circle.elim a p z = a) idp loop idp : sigma_sigma_eq_right
--     ... ≃ square idp idp (ap (circle.elim a p) loop) idp
--     : begin refine !eq_pathover_equiv_square ⬝e _, exact sorry end
--     ... ≃ p = idp : sorry

definition fn_circle_elim {A B : Type} (f : A → B) {a : A} (p : a = a) (z : S¹)
  : f (circle.elim a p z) = circle.elim (f a) (ap f p) z :=
begin
  induction z,
  { reflexivity},
  { apply eq_pathover, apply hdeg_square,
    exact ap_compose f _ _ ⬝ ap02 f !elim_loop ⬝ !elim_loop⁻¹}
end
...
namespace simple_two_quotient
  ...
  protected theorem rec_e {P : C → Type}
    ...
    : apdt (pre_rec Pj Pa Pe) (e s) = Pe s :=
  !rec_eq_of_rel
  ...
  begin
    ...
    rewrite [▸*,-apdt_eq_pathover_of_eq_ap,↑pre_elim,rec_e],
  end
  ...
  protected definition rec_et {P : C → Type}
    ...
    : apdt (pre_rec Pj Pa Pe) (et t) = e_closure.elimo e Pe t :=
  ...
  (surf_equiv_spokes (incltw r))⁻¹ᵉ ⟨aux q, λz, (fn_circle_elim i (et r) z)⁻¹ ⬝ incl2' q z⟩
      ...
      { apply pathover_pathover,
        ...
        { transitivity _,
          { apply ap (pathover_ap _ _),
            transitivity _, apply apdt_compose2 (pre_rec P0 _ _) (f q) loop,
            apply ap (pathover_of_pathover_ap _ _),
            transitivity _, apply apdt_change_path, exact !elim_loop⁻¹,
            transitivity _,
        ...
        -- rewrite [change_path_con, apdt_constant],
        ...
        -- apply whisker_bl_whisker_tl_eq
} end end},
  end
  ...
  theorem rec_incl1 {P : D → Type} (P0 : Π(a : A), P (incl0 a))
    ...
    : apdt (rec P0 P1 P2) (incl1 s) = P1 s :=
  begin
    unfold [rec, incl1], refine !apdt_ap ⬝ _, esimp, rewrite rec_e,
  ...
  theorem rec_inclt {P : D → Type} (P0 : Π(a : A), P (incl0 a))
    ...
    : apdt (rec P0 P1 P2) (inclt t) = e_closure.elimo incl1 P1 t :=
  ...
  begin
    ...
    xrewrite [naturality_apdt_eq (λx, !elim_eq_of_rel) loop],
...
namespace two_quotient
  ...
  theorem rec_incl1 {P : two_quotient → Type} (P0 : Π(a : A), P (incl0 a))
    ...
    (P2 : Π⦃a a' : A⦄ ⦃t t' : T a a'⦄ (q : Q t t'),
      change_path (incl2 q) (e_closure.elimo incl1 P1 t) = e_closure.elimo incl1 P1 t')
    ⦃a a' : A⦄ (s : R a a') : apdt (rec P0 P1 P2) (incl1 s) = P1 s :=
  rec_incl1 _ _ _ s
  ...
  theorem rec_inclt {P : two_quotient → Type} (P0 : Π(a : A), P (incl0 a))
    ...
    (P2 : Π⦃a a' : A⦄ ⦃t t' : T a a'⦄ (q : Q t t'),
      change_path (incl2 q) (e_closure.elimo incl1 P1 t) = e_closure.elimo incl1 P1 t')
    ⦃a a' : A⦄ (t : T a a') : apdt (rec P0 P1 P2) (inclt t) = e_closure.elimo incl1 P1 t :=
  rec_inclt _ _ _ t
