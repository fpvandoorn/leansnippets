...
section
  ...
  definition eq_constructors_eq {n m : ℕ} (a : A n) (a' : A m) (p : n = m) : i a = i a' :=
...
--    ... = i (add_comm m n ▸ fr n b)     : eq_constructors_same
    ... = i (fr n b)                    : !eq_constructors_eq !add_comm
...
-- eval apo011 @i idp (f_fr 1 b)
  ...
  theorem eq_constructors_eq_f {n m : ℕ} (a : A n) (a' : A m) (p : n = m) :
    (glue f a)⁻¹ ⬝ eq_constructors_eq (f a) (f a') (ap succ p) =
    eq_constructors_eq a a' p ⬝ (glue f a')⁻¹ :=
  by cases p; apply eq_constructors_same_f

  theorem eq_constructors_eq_f' {n m : ℕ} (a : A n) (a' : A m) :
    (glue f (fr m a))⁻¹ ⬝ eq_constructors_eq (fr (succ m) a) (fr n (f a')) (add_comm n (succ m))
    =
...
rewrite [con.assoc _ !i_fr !glue, i_fr_glue,
